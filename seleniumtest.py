...
def read_script(driver):
    ...
    fp1.close()
    wait = WebDriverWait(driver, 10)
    wait.until(EC.presence_of_element_located((By.CLASS_NAME, "searchresult_content")))#等待加载完成
...
def work_and_alter(driver,dcity,acity,date):
    search_city(driver,"dcity0",dcity)
    search_city(driver,"acity0",acity)
...
def main():
     ...
     fp=open("auxiliary.txt",mode="r")
     s=fp.readline()
     while(s):
         str=s.split(" ")
         print(str)
         work_and_alter(driver,str[0],str[1][0:-1], date)
         s=fp.readline()
...
#makeroom("20200810.txt")
main()
check_data("20200810.txt")
